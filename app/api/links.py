...
from app.models.schemas import LinkCreate, Link, LinkUpdate
...
@router.get("/links/{short_code}", response_class=RedirectResponse, status_code=307)
...
@router.get("/links/{short_code}/stats", response_model=Link)
async def get_link_info(short_code: str, db: Session = Depends(get_db)):
    """Get information about a specific link."""
    link = get_link_by_short_code(db, short_code, is_redirect=False)
    if not link or not link.is_active:
        raise HTTPException(status_code=404, detail="Link not found or expired")
